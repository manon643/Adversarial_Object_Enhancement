...
def image_chipper(parent_img_name, chip_size, output_dir):
    ...
    for k in range(10):
        ...
        random_i = np.random.randint(0, w-chip_size+1)
        random_j = np.random.randint(0, h-chip_size+1)
        img_chip = img_arr[random_i:random_i+chip_size, random_j:random_j+chip_size, :]
        ...
        yield chip_name, [random_i, random_j, random_i+chip_size, random_j+chip_size]
    ...
    for obj in ground_truth:
        ...
        if bbox[0]>= coords[0] and bbox[1]>= coords[1]:
            if bbox[2]<= coords[2] and bbox[3]<= coords[3]:
                x_min = (float(bbox[0]) - offset_col) / float(chip_size)
                y_min = (float(bbox[1]) - offset_row) / float(chip_size)
                x_max = (float(bbox[2]) - offset_col) / float(chip_size)
                y_max = (float(bbox[3]) - offset_row) / float(chip_size)
